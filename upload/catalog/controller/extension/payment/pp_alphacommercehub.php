...
class ControllerExtensionPaymentPPAlphacommercehub extends Controller {
	...
	public function callback() {
		...
		if ($order_info) {
			...
			if ($this->config->get('pp_alphacommercehub_debug')) {
			...
			if ((strcmp($response, 'VERIFIED') == 0 || strcmp($response, 'UNVERIFIED') == 0) && isset($this->request->post['payment_status'])) {
				...
				switch($this->request->post['payment_status']) {
					case 'Canceled_Reversal':
						$order_status_id = $this->config->get('pp_alphacommercehub_canceled_reversal_status_id');
						break;
					case 'Completed':
						$receiver_match = (strtolower($this->request->post['receiver_email']) == strtolower($this->config->get('pp_alphacommercehub_email')));
						...
						if ($receiver_match && $total_paid_match) {
							$order_status_id = $this->config->get('pp_alphacommercehub_completed_status_id');
						}
						...
						break;
					case 'Denied':
						$order_status_id = $this->config->get('pp_alphacommercehub_denied_status_id');
						break;
					case 'Expired':
						$order_status_id = $this->config->get('pp_alphacommercehub_expired_status_id');
						break;
					case 'Failed':
						$order_status_id = $this->config->get('pp_alphacommercehub_failed_status_id');
						break;
					case 'Pending':
						$order_status_id = $this->config->get('pp_alphacommercehub_pending_status_id');
						break;
					case 'Processed':
						$order_status_id = $this->config->get('pp_alphacommercehub_processed_status_id');
						break;
					case 'Refunded':
						$order_status_id = $this->config->get('pp_alphacommercehub_refunded_status_id');
						break;
					case 'Reversed':
						$order_status_id = $this->config->get('pp_alphacommercehub_reversed_status_id');
						break;
					case 'Voided':
						$order_status_id = $this->config->get('pp_alphacommercehub_voided_status_id');
						break;
				}
